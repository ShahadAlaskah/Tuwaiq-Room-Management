...
public class FloorsController : ControllerBase
{
    ...
    public async Task<IActionResult> Get(int? page = 1, int? pageSize = 10, string? query = null)
    {
        ...
        var result = await _sender.Send(new GetFloorQuery(new GetFloorSpecification(query), page ?? 1, pageSize ?? 10));
        ...
        return Ok(new ApiResponse<FloorDto>
        {
            Pagination = new()
            {
    ...
    [ProducesResponseType(typeof(Error), StatusCodes.Status400BadRequest)]
    [HttpGet("[action]/{id}")]
    public async Task<IActionResult> Get(FloorId id)
    {
        var result = await _sender.Send(new GetFloorQuery(new GetFloorSpecification(id), 1, 1));
        if (result.Failure) return BadRequest(result.Error);
    ...
    [ProducesResponseType(typeof(Error), StatusCodes.Status400BadRequest)]
    [HttpPost("[action]")]
    public async Task<IActionResult> Post(CreateFloorCommand command)
    {
        var result = await _sender.Send(command);
        if (result.Failure) return BadRequest(result.Error);

        var toReturn = await _sender.Send(new GetFloorQuery(new GetFloorSpecification(result.Value), 1, 1));
    ...
    [ProducesResponseType(typeof(Error), StatusCodes.Status400BadRequest)]
    [HttpPut("[action]")]
    public async Task<IActionResult> Put(UpdateFloorCommand command)
    {
        var result = await _sender.Send(command);
        if (result.Failure) return BadRequest(result.Error);
        return Accepted();
    ...
    public async Task<IActionResult> Delete(FloorId id)
    {
        var result = await _sender.Send(new DeleteFloorCommand(id));
        if (result.Failure) return BadRequest(result.Error);
        return NoContent();
    }
}